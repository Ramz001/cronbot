...
export const cacheKeys = {
  integrationToken: (userId: string) => `${CACHE_KEYS.INTEGRATION_TOKEN}:${userId}`,
  integrationTokenCount: (userId: string) => `${CACHE_KEYS.INTEGRATION_TOKEN_COUNT}:${userId}`,
  ...
  discordChannel: (guildId: string) => `${CACHE_KEYS.DISCORD_CHANNELS}:${guildId}`,
} as const;
